<nav x-data="{ open: false }" class="bg-dark dark:bg-gray-800 border-b border-gray-700 fixed w-full top-0 z-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            <div class="flex items-center">
                <div class="shrink-0 flex items-center">
                    <x-application-logo style="width: 100px; height: 60px;" class="block fill-current text-white" />
                </div>
            </div>
            ...
            <div class="hidden sm:flex sm:items-center">
                @if (Auth::check())
                    <x-dropdown aling="right" width="48">
                        ...
                        <x-slot name="content">
                            <x-dropdown-link :href="route('profile.edit')" class="text-black no-underline">
                                {{ __('Profile') }}
                            </x-dropdown-link>
                            ...
                            <form method="POST" action="{{ route('logout') }}">
                                ...
                                <x-dropdown-link :href="route('logout')"
                                        onclick="event.preventDefault();
                                                    this.closest('form').submit();" class="text-black no-underline">
                                    {{ __('Log Out') }}
                                </x-dropdown-link>
                            </form>
                        </x-slot>
